...
def render_animation_dual(data_real, data_gen, skeleton, fps, output='interactive', bitrate=1000,zoom=False):
    """
    Render or show an animation. The supported output modes are:
     -- 'interactive': display an interactive figure
                       (also works on notebooks if associated with %matplotlib inline)
    ...
    if zoom != False:
        x1,x2,y1,y2,z1,z2 = zoom
        ax_real.set_xlim3d([x1*radius, x2*radius])
        ax_real.set_zlim3d([z1*radius, z2*radius])
        ax_real.set_ylim3d([y1*radius, y2*radius])
    else:
    ...
    def update_single(frame, ax, data, data_other, spline_line, lines, trajectory, initialized,lines_elbow_last_point,lines_elbow_last_point_other,index_for_joint):
        ...
        positions_world = data[frame]
        positions_world_other = data_other[frame]
        for i in range(positions_world.shape[0]):
            if skeleton_parents[i] == -1:
                continue
            if not initialized:
                ...
                lines[i-1][0].set_3d_properties([positions_world[i, z], positions_world[skeleton_parents[i], z]], zdir='y')
        if not initialized:
            lines_elbow_last_point[0] = [positions_world[index_for_joint,x],positions_world[index_for_joint,y],positions_world[index_for_joint,z]]
            lines_elbow_last_point_other[0] = [positions_world_other[index_for_joint,x],positions_world_other[index_for_joint,y],positions_world_other[index_for_joint,z]]
        else:
            ax.plot([lines_elbow_last_point[0][0],positions_world[index_for_joint,x]],
                [lines_elbow_last_point[0][1],positions_world[index_for_joint,y]],
                [lines_elbow_last_point[0][2],positions_world[index_for_joint,z]],zdir='y',c='blue',linewidth=0.5)
            ax.plot([lines_elbow_last_point_other[0][0],positions_world_other[index_for_joint,x]],
                [lines_elbow_last_point_other[0][1],positions_world_other[index_for_joint,y]],
                [lines_elbow_last_point_other[0][2],positions_world_other[index_for_joint,z]],zdir='y',c='red',linewidth=0.5)
            lines_elbow_last_point[0] = [positions_world[index_for_joint,x],positions_world[index_for_joint,y],positions_world[index_for_joint,z]]
            lines_elbow_last_point_other[0] = [positions_world_other[index_for_joint,x],positions_world_other[index_for_joint,y],positions_world_other[index_for_joint,z]]
    ...
    def update(frame):
        nonlocal initialized_real, lines_real_elbow_last_point, index_for_joint
        nonlocal initialized_gen, lines_gen_elbow_last_point
        update_single(frame, ax_real, data_real, data_gen, spline_line_real, lines_real, trajectory_real, initialized_real, lines_real_elbow_last_point, lines_gen_elbow_last_point, index_for_joint)
        update_single(frame, ax_gen, data_gen, data_real, spline_line_gen, lines_gen, trajectory_gen, initialized_gen, lines_gen_elbow_last_point, lines_real_elbow_last_point, index_for_joint)
